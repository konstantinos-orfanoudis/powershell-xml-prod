...
# Default paths matching the generate-oim-template-tests skill defaults
$script:DefaultTestAssistRoot  = "C:\Users\aiuser\Desktop\DeploymentManager-4.0.6-beta\TestAssist-distribute\TestAssist-1.2.2"
$script:DefaultProjectConfigPath = "C:\Users\aiuser\Documents\DMWorkshop2\Config\Example\projectConfig.xml"

function Get-OneImAuditSessionSettings {
    <#
    .SYNOPSIS
    Returns resolved TestAssist root and project config paths.
    Falls back to known defaults when parameters are not supplied.
    #>
    param(
        [Parameter()]
    ...
    return [pscustomobject]@{
        ...
        SetupPath         = Join-Path $resolvedTestAssistRoot "Setup.ps1"
        ModulePath        = Join-Path $resolvedTestAssistRoot "Intragen.TestAssist.psd1"
    }
}
