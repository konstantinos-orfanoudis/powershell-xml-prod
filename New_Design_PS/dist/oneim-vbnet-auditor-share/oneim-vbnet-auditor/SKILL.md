...
Use only these online authorities for evidence that can affect findings:

1. One Identity official docs on `support.oneidentity.com`
2. Microsoft Learn on `learn.microsoft.com`
3. OWASP cheat sheets and OWASP community guidance on `cheatsheetseries.owasp.org` and `owasp.org`

Evidence priority is fixed:

1. One Identity for OneIM behavior and version context
2. Microsoft Learn for VB.NET and .NET semantics, analyzers, reliability, and performance
3. OWASP for vulnerability classes where vendor guidance is not specific enough
