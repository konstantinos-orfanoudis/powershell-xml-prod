...
function Get-OneImAuditPesterModulePath {
    $candidates = @(
        (Join-Path $HOME "Documents\WindowsPowerShell\Modules\Pester\4.10.1\Pester.psd1"),
        (Join-Path $HOME "Documents\PowerShell\Modules\Pester\4.10.1\Pester.psd1"),
        (Join-Path ${env:ProgramFiles} "WindowsPowerShell\Modules\Pester\4.10.1\Pester.psd1"),
        (Join-Path ${env:ProgramFiles} "PowerShell\Modules\Pester\4.10.1\Pester.psd1")
...
if ($null -eq $pwshCmd) {
    ...
    $pester4Path = Get-OneImAuditPesterModulePath
    $escapedPester4Path = if ($pester4Path) { $pester4Path -replace "'", "''" } else { "" }
    ...
    foreach ($entry in $ps7Tests) {
        ...
        & $pwshExe -NoProfile -NonInteractive -Command "
            `$ErrorActionPreference = 'Stop'
            if (Test-Path -LiteralPath '$escapedPester4Path') {
                Import-Module '$escapedPester4Path' -Force
            } else {
